    ...
    switch (keycode) {
        case KC_F13:
            if (record->event.pressed) {
                SEND_STRING(SS_LCTL("lc") SS_TAP(X_LGUI) "pwsh" SS_TAP(X_ENT) SS_DELAY(500) "mpv \"" SS_LCTL("v") "\"" SS_TAP(X_ENT)); 
            }
        break;
    }
    return true;
...
bool encoder_update_user(uint8_t index, bool clockwise) {
    ...
    static void render_name(void) {
        static const char PROGMEM logo[] = {
            ...
            0xD9, 0xD9, 0xD9, 0xD9, 0xD9, 0xD9, 0x00
        };
        oled_write_P(logo, false);
    }

    static void render_logo(void) {     // Render NERV Logo
        static const char PROGMEM logo_1[] = {0x9A, 0x9B, 0x9C, 0x9D, 0x9E, 0x9F, 0x00};
        static const char PROGMEM logo_2[] = {0xBA, 0xBB, 0xBC, 0xBD, 0xBE, 0xBF, 0x00};
        static const char PROGMEM logo_3[] = {0xDA, 0xDB, 0xDC, 0xDD, 0xDE, 0xDF, 0x00};
        static const char PROGMEM logo_4[] = {0xD9, 0xD9, 0xD5, 0xD6, 0xD7, 0xD8, 0x00};
        oled_set_cursor(0,0);
    ...
    void oled_task_user(void) {
        ...
        if ( IS_HOST_LED_OFF(USB_LED_NUM_LOCK) && IS_HOST_LED_OFF(USB_LED_CAPS_LOCK) && \
             IS_HOST_LED_OFF(USB_LED_SCROLL_LOCK) && get_highest_layer(layer_state) == 0 ) {
            render_name();
            clear_screen = true;
        } else {
            ...
            switch (get_highest_layer(layer_state)) {
                ...
                case 3:
                    ...
                    break;
                case 4:
                    oled_write_P(PSTR("Layer 5   "), false);
                    break;
                case 5:
                    oled_write_P(PSTR("Layer 6   "), false);
                    break;
            }
